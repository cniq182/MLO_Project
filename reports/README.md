...
The total code coverage of our project is 64%, as measured using pytest together with the pytest-cov plugin, when focusing on the most critical source files: api.py, data.py, and model.py. These components are central to the system’s functionality, as they handle data preprocessing, model behavior, and inference through the API. For this reason, the testing effort was intentionally concentrated on these files to ensure correctness in the parts of the codebase most directly affecting the end user.When including all source files under src/, the overall coverage drops to approximately 31%.
